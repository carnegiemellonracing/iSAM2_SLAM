...
std::vector<New_cone_info> slamISAM::sort_cone_ids(const std::vector<gtsam::Point2> &color_slam_est, std::vector<New_cone_info> &new_cones) {
    ...
    const int REVISION_THRESHOLD = 20;
    ...
    bool revised_initial = false;

    while (remaining > 0) {
        // Phase 1: Initial collection of cones
        if (!revised_initial && ordered.size() < REVISION_THRESHOLD) {
            ...
            // Trigger revision when threshold reached
            if (ordered.size() == REVISION_THRESHOLD) {
                // Phase 2: Re-process first 20 cones backwards
                std::vector<New_cone_info> temp_ordered;
                std::vector<int> temp_indices;
                gtsam::Point2 revision_current = current;
                double revision_angle = prev_angle;

                // Create index mapping for first 20 cones
                for (const auto& cone : ordered) {
                    for (int i = 0; i < new_cones.size(); ++i) {
                        if (new_cones[i].local_cone_pos == cone.local_cone_pos) {
                ...
                // Reverse process from last cone back
                for (int steps = 0; steps < REVISION_THRESHOLD; ++steps) {
                    double min_score = DBL_MAX;
                    int best_idx = -1;
                    ...
                    for (int i : temp_indices) {
                        ...
                        const auto &candidate = new_cones[i];
                        double dx = revision_current.x() - candidate.local_cone_pos.x();
                        double dy = revision_current.y() - candidate.local_cone_pos.y();
                        double distance = std::hypot(dx, dy);
                        double angle = std::atan2(dy, dx);
                        double angle_diff = std::abs(angle - revision_angle);
                        angle_diff = std::min(angle_diff, 2 * M_PI - angle_diff);
                        double score = 0.7 * distance + 0.3 * angle_diff;
                    ...
                    if (best_idx != -1) {
                        temp_ordered.insert(temp_ordered.begin(), new_cones[best_idx]);
                        used[best_idx] = false;  // Temporarily release
                        revision_current = new_cones[best_idx].local_cone_pos;
                        
                        if (temp_ordered.size() >= 2) {
                            const auto &next_pos = temp_ordered[1].local_cone_pos;
                            revision_angle = std::atan2(next_pos.y() - revision_current.y(),
                                                      next_pos.x() - revision_current.x());
                        }
                    }
                }

                // Replace initial ordering with revised version
                ordered = temp_ordered;
                for (int i : temp_indices) used[i] = true;  // Re-mark as used
                revised_initial = true;
                
                // Reset tracking variables for continued processing
                current = ordered.back().local_cone_pos;
                prev_angle = std::atan2(current.y() - ordered.rbegin()[1].local_cone_pos.y(),
                                      current.x() - ordered.rbegin()[1].local_cone_pos.x());
            }
        }
        // Phase 3: Continue normal processing
        else {
            // Standard scoring logic from original implementation
            double min_score = DBL_MAX;
            int best_idx = -1;
