...
void data_association_allobs(int n_obs, int n_landmarks, MatrixXd &global_cone_x, MatrixXd &global_cone_y, vector<double> &m_dist,
                        vector<Point2> &cone_obs, vector<Point2> &slam_est, vector<MatrixXd> &slam_mcov)
{
    ...
    for (int o = 0; o < n_obs; o++)
    {
        for (int i = 0; i < n_landmarks; i++)
        {
            ...
            c(0, i + o * n_landmarks) = diff.x();
            c(1, i + o * n_landmarks) = diff.y();
        }
    }
    // element multiplication
    MatrixXd dists = a.array() * b.array();
    VectorXd dists_vec = dists.colwise().sum();
    MatrixXd dists_mat = Eigen::Map<Eigen::MatrixXd>(dists_vec.data(), 2, n_landmarks * n_obs * 2);
    dists_mat.array() = dists_mat.array() * c.array();
    VectorXd m_dist_vec = dists_mat.colwise().sum();
    for (int i = 0; i < n_landmarks; i++)
    {
...
NEW
    ...
    void
    data_association_3(int n_obs, int n_landmarks, MatrixXd &global_cone_x, MatrixXd &global_cone_y, vector<double> &m_dist,
                       vector<Point2> &cone_obs, vector<Point2> &slam_est, vector<MatrixXd> &slam_mcov)
{   
    Eigen::MatrixXd a = Eigen::MatrixXd::Zero(2, n_landmarks * n_obs * 2);
    Eigen::MatrixXd b(2, n_landmarks * n_obs * 2);
